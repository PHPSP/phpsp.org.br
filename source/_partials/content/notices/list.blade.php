<div class="card">
    <div class="card-content">
        <p class="subtitle">
            Salve pessoal! Além dos eventos mensais PHPSP+Pub em São Paulo e em Campinas temos mais eventos sendo
            construídos com a ajuda da comunidade e dos parceiros. Faça parte da comunicade e ajude a evangelizar
            o PHPSP!
        </p>
    </div>
    <footer class="card-footer">
        <p class="card-footer-item">
          <span>
            <a href="{{ $page->links->twitter->url }}">Twitter</a>
          </span>
        </p>
        <p class="card-footer-item">
          <span>
            <a href="{{ $page->links->facebook->url }}">Facebook</a>
          </span>
        </p>
        <p class="card-footer-item">
          <span>
            <a href="{{ $page->links->slack->url }}">Slack</a>
          </span>
        </p>
    </footer>
</div>
<hr>
<div class="card">
    <div class="card-content">
        <p class="subtitle">
            Ajude-nos a divulgar e a criar mais e mais conteúdos interessantes do mundo PHP, colabore com conteúdos e com projetos e ajude a evangelizar
            o PHPSP!
        </p>
    </div>
    <footer class="card-footer">
        <p class="card-footer-item">
          <span>
            <a href="/como-contribuir">Como Contribuir?</a>
          </span>
        </p>
        <p class="card-footer-item">
          <span>
            <a href="{{ $page->links->github->url }}">Github</a>
          </span>
        </p>
    </footer>
</div>
